...
// Scan ttf codepoints from memory loaded file data
void ScanCpts(unsigned char* fileData, int dataSize, int** codepoints, int* codepointCount)
{
    int count = 0;
    if (fileData != NULL) {
        ...
        {
            // 0 - code of some definitely unsupported char in my case
            int replacer = stbtt_FindGlyphIndex(&fontInfo, 0);
            for (int i = 0; i < 65536; i++) {
                int g = stbtt_FindGlyphIndex(&fontInfo, i);                
                if (g != replacer) {
                    int chw,chh,offsetX,offsetY,x1,y1,x2,y2;
                    stbtt_GetGlyphBitmapBox(&fontInfo,g,1.0,1.0,&x1,&y1,&x2,&y2);
                    float scaleX=16.0/fabs(x2-x1);
                    float scaleY= 16.0/fabs(y2-y1);
                    char* data=stbtt_GetCodepointBitmap(&fontInfo, scaleX, scaleY, i, &chw, &chh, &offsetX, &offsetY);
                    bool ok=0;
                    for(int l=0;l<chw*chh;++l)
                    {
                        if(data[l]>0) {ok=1;break;}
                    }
                    if(ok)
                    {
                    (*codepoints)[count] = i;
...
// Scan ttf foin for defined codepoints from file
void ScanCptsFile(const unsigned char* fileName, int** codepoints, int* codepointCount)
{
    int dataSize = 0;
    unsigned char* fileData = LoadFileData(fileName, &dataSize);
    if (fileData != NULL) {
        ScanCpts(fileData, dataSize, codepoints, codepointCount);
        UnloadFileData(fileData);
    }
    int count = (*codepointCount);
    printf("found %d codepoints\n", count);
}
...
#define COLOR_EQUAL(col1, col2) ((col1.r == col2.r) && (col1.g == col2.g) && (col1.b == col2.b) && (col1.a == col2.a))
    ...
    while ((lineSpacing + lineToRead * (charHeight + lineSpacing)) < image.height) {
        while ((xPosToRead < image.width) && !COLOR_EQUAL((pixels[(lineSpacing + (charHeight + lineSpacing) * lineToRead) * image.width + xPosToRead]), key)) {
    ...
    if (TextIsEqual(fileExtLower, ".ttf") || TextIsEqual(fileExtLower, ".otf")) {
        ...
        if (TextIsEqual(fileExtLower, ".ttf")) {
            if (codepoints == NULL) {
                ...
                LoadFontData(fileData, dataSize, font.baseSize, codepoints, codepointCount, FONT_DEFAULT);
            }
        }
        if (font.glyphs != NULL) {
...
#define FONT_BITMAP_ALPHA_THRESHOLD 0 // Bitmap (B&W) font generation alpha threshold
#endif
            ...
            for (int i = 0; i < codepointCount; i++) {
                ...
                if (type != FONT_SDF) {
                    chars[i].image.data =
                        ...
                        //stbtt_GetCodepointBitmapSubpixel(&fontInfo,1.0,1.0,0,0,ch, &chw, &chh, &chars[i].offsetX, &chars[i].offsetY);
                } else if (ch != 32) {
                ...
                if (type == FONT_BITMAP) {
                    ...
                    if(FONT_BITMAP_ALPHA_THRESHOLD>0)
...
#define MAX_FONT_DATA_SIZE 1024 * 1024 * 10 // 1 MB
...
#define SUPPORT_UNORDERED_CHARSET 1
